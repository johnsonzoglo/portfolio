...
function renderStreams(){
  ...
  $('#streamsTable').innerHTML=visible.length?`<div class="stream-row header"><span>Resource</span><span>Type</span><span>Level</span><span>Status</span><span>Access</span><span></span></div>${visible.map(stream=>`<div class="stream-row"><div class="stream-cell">${stream.src?`<video class="stream-preview" src="${esc(stream.src)}" ${stream.coverImage?`poster="${esc(stream.coverImage)}"`:''} preload="metadata" muted></video>`:stream.coverImage?`<img class="stream-preview" src="${esc(stream.coverImage)}" alt="">`:`<span class="stream-preview stream-fallback"><i data-lucide="${stream.contentType==='course'?'book-open':'play'}"></i></span>`}<div><h3>${esc(stream.title)}</h3><small>${esc(stream.game)}</small></div></div><span>${stream.contentType==='course'?'Course':'Video lesson'}</span><span>${esc(stream.level||'All levels')}</span><span class="visibility-pill ${stream.published===false?'hidden':''}">${stream.published===false?'Draft':stream.publishAt&&new Date(stream.publishAt)>new Date()?'Scheduled':'Published'}</span><span>${stream.accessType==='paid'?`<b class="feature-pill">Paid · ${money(stream.price)}</b>`:stream.featured?'<b class="feature-pill">Free · Featured</b>':'Free'}</span><div class="table-actions"><button class="icon-button" data-edit-stream="${esc(stream.id)}" title="Edit resource" aria-label="Edit ${esc(stream.title)}"><i data-lucide="pencil"></i></button><button class="icon-button danger" data-delete-stream="${esc(stream.id)}" title="Delete resource" aria-label="Delete ${esc(stream.title)}"><i data-lucide="trash-2"></i></button></div></div>`).join('')}`:emptyState('graduation-cap','No academy content found',streams.length?'Try a different search or filter.':'Add your first course or video lesson.');
  icons();
}
...
function renderModules(modules){
  $('#moduleList').innerHTML=(modules||[]).map((module,moduleIndex)=>`<section class="course-module" data-module-index="${moduleIndex}" data-module-id="${esc(module.id||'')}"><div class="module-head"><input class="module-title" value="${esc(module.title||`Module ${moduleIndex+1}`)}" placeholder="Module title"><div class="module-actions"><button type="button" data-module-move="-1" title="Move up">↑</button><button type="button" data-module-move="1" title="Move down">↓</button><button type="button" data-remove-module title="Remove">×</button></div></div><div class="lesson-list">${(module.lessons||[]).map((lesson,lessonIndex)=>lessonRow(lesson,lessonIndex)).join('')}</div><button class="add-lesson" type="button" data-add-lesson>+ Add lesson</button></section>`).join('');
}
function lessonRow(lesson={},index=0){return `<div class="lesson-row" data-lesson-id="${esc(lesson.id||'')}"><input class="lesson-title" value="${esc(lesson.title||`Lesson ${index+1}`)}" placeholder="Lesson title"><input class="lesson-duration" value="${esc(lesson.duration||'')}" placeholder="Duration"><input class="lesson-video" value="${esc(lesson.video||'')}" placeholder="Video URL"><input class="lesson-resource" value="${esc(lesson.resource||'')}" placeholder="PDF / download URL"><button class="lesson-remove" type="button" data-remove-lesson>×</button><textarea class="lesson-description" rows="2" placeholder="Lesson description">${esc(lesson.description||'')}</textarea></div>`}
...
$('#streamFile').addEventListener('change',event=>{$('#streamFileName').textContent=event.target.files[0]?.name||'Choose lesson video'});
...
$('#streamType').addEventListener('change',()=>{$('#courseBuilder').hidden=$('#streamType').value!=='course'});
...
$('#moduleList').addEventListener('click',event=>{const moduleElement=event.target.closest('.course-module');if(!moduleElement)return;let modules=collectModules(),index=Number(moduleElement.dataset.moduleIndex);if(event.target.closest('[data-add-lesson]'))modules[index].lessons.push({title:`Lesson ${modules[index].lessons.length+1}`});if(event.target.closest('[data-remove-module]'))modules.splice(index,1);if(event.target.closest('[data-remove-lesson]')){const lessonIndex=[...moduleElement.querySelectorAll('.lesson-row')].indexOf(event.target.closest('.lesson-row'));modules[index].lessons.splice(lessonIndex,1)}const move=event.target.closest('[data-module-move]');if(move){const next=index+Number(move.dataset.moduleMove);if(next>=0&&next<modules.length)[modules[index],modules[next]]=[modules[next],modules[index]]}renderModules(modules)});
$('#streamForm').addEventListener('submit',async event=>{
  event.preventDefault();const id=$('#streamId').value,file=$('#streamFile').files[0],coverFile=$('#streamCoverPhoto').files[0],message=$('#streamEditorMessage'),button=event.submitter,payload={title:$('#streamTitle').value,contentType:$('#streamType').value,accessType:$('#streamAccess').value,price:Number($('#streamPrice').value)||0,game:$('#streamGame').value,level:$('#streamLevel').value,date:$('#streamDate').value,publishAt:$('#streamPublishAt').value?new Date($('#streamPublishAt').value).toISOString():'',duration:$('#streamDuration').value,lessons:Number($('#streamLessons').value)||1,description:$('#streamDescription').value,coverImage:$('#streamCoverImage').value,modules:collectModules(),published:$('#streamPublished').checked,featured:$('#streamFeatured').checked};message.textContent='';button.disabled=true;$('#streamUploadMeter').hidden=Boolean(id)||!file;
  try{
    ...
    if(id)await api(`/api/admin/streams/${id}`,{method:'PATCH',body:JSON.stringify(payload)});
      ...
      if(!file)await api('/api/admin/academy-resources',{method:'POST',body:JSON.stringify(payload)});
      else{const type=file.type||({mp4:'video/mp4',webm:'video/webm',ogg:'video/ogg',ogv:'video/ogg',mov:'video/quicktime'}[file.name.split('.').pop().toLowerCase()]);const params=new URLSearchParams({...payload,modules:JSON.stringify(payload.modules),published:String(payload.published),featured:String(payload.featured)}),response=await fetch(`/api/admin/streams?${params}`,{method:'POST',headers:{Authorization:`Bearer ${token}`,'Content-Type':type||'application/octet-stream'},body:file}),result=await response.json().catch(()=>({}));if(response.status===401){signOut();throw new Error(result.error||'Your session expired.')}if(!response.ok)throw new Error(result.error||'Upload failed.')}
    }
